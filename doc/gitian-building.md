...
*Setup instructions for a Gitian build of HCC Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HCC
...
- [Building HCC Core](#building-hcc-core)
...
Clone the git repositories for HCC Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hcccrypto/hcc
...
in the HCC Core repository under 'Fetch and create inputs' to install sources which require
...
To build HCC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the HCC Core repository.
...
```bash
    ...
    From https://github.com/hcccrypto/hcc
...
For example:
```bash
URL=https://github.com/crowning-/hcc.git
...
and the hcc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hcc/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url hcc=${BTCPATH},signature=${SIGPATH} ../hcc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hcc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hcc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hcc-osx-build.assert
```
