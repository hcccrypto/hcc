Cross-compiliation of HCC Core
===============================

HCC Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
...
Note that for WSL the HCC Core source path MUST be somewhere in the default mount file system, for
example /usr/src/hcc, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the HCC Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the HCC Core source code).
